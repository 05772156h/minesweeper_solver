...
def get_place(mine_list, count_list, already_chosen_count_list, num, x_0, correct):
    x = x_0
    j_0 = -1
    len_count_list = len(count_list)
    for i in range(num):
        if x % len_count_list <= j_0:
            correct = False
            break
        place = count_list[x % len_count_list]
        if not place in mine_list and place in already_chosen_count_list:
            correct = False
            break
        elif not place in mine_list:
            mine_list.append(place)
        j_0 = x % len_count_list
        x = int((x - x % len_count_list) / len_count_list)

    for place in count_list:
        if not place in already_chosen_count_list:
            already_chosen_count_list.append(place)
    return correct, mine_list, already_chosen_count_list
...
def cal_prob():
    ...
    if out == True:
        ...
        x_max = 1
        for i in range(len(num_list)):
            x_max *= len(count_list_list[i]) ** num_list[i]
        # print(x_max)
        # print(len(num_list))
        ...
        for i in range(x_max):
            ...
            correct = True
            mine_list = []
            already_chosen_count_list = []
            for j in range(len(num_list)):
                if x_list[j] == len(count_list_list[j]) ** num_list[j]:
                    x_list[j] = 0
                    x_list[j + 1] += 1
                ...
                if correct == False:
                    ...
                    correct, mine_list, already_chosen_count_list = get_place(mine_list, count_list_list[j], already_chosen_count_list, num_list[j], x_list[j], correct)
            x_list[0] += 1
        ...
        #     correct = True
        #     x_list.append(0)
        # for i in range(2 ** len(total_count_list)):
        #     print(i + 1)
        #     mine_list = []
        ...
        #     for j in range(len(total_count_list)):
        #         if x_list[j] == 1:
        #             mine_list.append(total_count_list[j])
        #     x_list[0] += 1
